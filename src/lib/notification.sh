...
if [ "${update_number}" -eq 1 ]; then
	if [ -z "${last_notif_id}" ]; then
		# shellcheck disable=SC2154
		notify-send -p -a "Cachy-Update" -n "cachy-update_updates-available-${tray_icon_style}${colorblind_mode}" "Cachy-Update" "$(eval_gettext "1 update available")" -A "run=$(eval_gettext "Run Cachy-Update")" -A "close=$(eval_gettext "Close")" > "${tmpdir}/notif_param"
	else
		# shellcheck disable=SC2154
		notify-send -p -r "${last_notif_id}" -a "Cachy-Update" -n "cachy-update_updates-available-${tray_icon_style}${colorblind_mode}" "Cachy-Update" "$(eval_gettext "1 update available")" -A "run=$(eval_gettext "Run Cachy-Update")" -A "close=$(eval_gettext "Close")" > "${tmpdir}/notif_param"
	fi
else
	if [ -z "${last_notif_id}" ]; then
